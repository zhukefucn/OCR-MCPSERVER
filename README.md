...
the API key only through `OCR_VERIFY_API_KEY` and the Git-SHA/three-image
candidate identity through the required `--run-id`. It emits bounded
content-free JSON and never creates image tags.
